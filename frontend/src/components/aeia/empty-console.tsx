...
export function EmptyConsole({ onSelectPrompt }: EmptyConsoleProps) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-8 max-w-2xl mx-auto my-auto select-none font-mono animate-in fade-in duration-300">
      {/* Brand Hero */}
      <div className="flex size-12 items-center justify-center rounded-2xl bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 mb-4 shadow-lg shadow-emerald-500/5">
        ...
        {SUGGESTIONS.map((item) => {
          ...
          return (
            <button
              ...
              className="group flex items-start gap-3 p-3 text-left rounded-xl border border-white/8 bg-[#0e0e12] hover:bg-zinc-900/80 hover:border-emerald-500/30 transition shadow-sm"
            >
              <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-zinc-800/60 border border-white/6 text-zinc-400 group-hover:text-emerald-400 group-hover:border-emerald-500/30 transition">
